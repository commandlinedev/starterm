...
Star's configuration files are located at `~/.config/starterm/`.

The main configuration file is `settings.json` (`~/.config/starterm/settings.json`).
...
The easiest way to edit your config files is to use the wsh editconfig command which will open your Star config file in our built-in preview editor.
...
| ------------------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| app:globalhotkey                     | string   | A systemwide keybinding to open your most recent star window. This is a set of key names separated by `:`. For more info, see [Customizable Systemwide Global Hotkey](#customizable-systemwide-global-hotkey)                                                 |
| app:dismissarchitecturewarning       | bool     | Disable warnings on app start when you are using a non-native architecture for Star. For more info, see [Why does Star warn me about ARM64 translation when it launches?](./faq#why-does-star-warn-me-about-arm64-translation-when-it-launches).              |
...
| ai:name                              | string   | string to display in the Star AI block header                                                                                                                                                                                                                 |
...
If you installed Star pre-v0.9.0 your configuration file will be located at
`~/.starterm/config/settings.json`. This includes all of the other configuration
...
```json
{
  ...
  "bookmark@star-github": {
    "url": "https://github.com/commandlinedev/starterm",
    "title": "Star Github",
  ...
  "bookmark@star-pulls": {
    "url": "https://github.com/commandlinedev/starterm/pulls",
    "title": "Star Pull Requests",
    "iconurl": "https://github.githubassets.com/favicons/favicon-dark.png"
  }
}
```
...
User-defined terminal themes are located in `~/.config/starterm/termthemes.json`.
...
Here is an example of defining a full terminal theme. All of the built-in themes are defined here: https://github.com/commandlinedev/starterm/blob/main/pkg/wconfig/defaultconfig/termthemes.json (if you'd like to add a popular terminal theme, please submit a PR!)
...
Star allows settings a custom global hotkey to open your most recent window from anywhere in your computer. This has the name `"app:globalhotkey"` in the `settings.json` file and takes the form of a series of key names separated by the `:` character.

### Examples

As a practical example, suppose you want a value of `F5` as your global hotkey. Then you can simply set the value of `"app:globalhotkey"` to `"F5"` and reboot Star to make that your global hotkey.
