...
Star Terminal is a modern terminal that includes graphical capabilities like web browsing, file previews, and AI assistance alongside traditional terminal features. This guide will help you get started.
...
ARM64 is planned, but is currently blocked by upstream dependencies (see [Windows ARM Support](https://github.com/commandlinedev/starterm/issues/928)).
...
```powershell
winget install CommandLine.Star
```
...
```bash
sudo snap install --classic starterm
```

Other options available: [AUR package](https://aur.archlinux.org/packages/starterm) (community maintained), [Nix package](https://search.nixos.org/packages?channel=unstable&show=starterm) (community maintained)

</PlatformItem>

You can also download installers directly from our [Downloads page](https://www.starterm.dev/download).
...
- **Blocks**: The building blocks of Star. Each block can be a terminal, web browser, file preview, AI chat, or other widget.
...
1. **Terminal Features**
   ...
   - Includes the `wsh` command for interacting with Star's GUI features
...
1. **Open Your First New Tab**
   ...
   - New Star tabs start with a single terminal block
...
- Report issues on [GitHub](https://github.com/commandlinedev/starterm/issues)
