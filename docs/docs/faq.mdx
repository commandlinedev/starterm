...
### How do I configure Star to use different AI models/providers?

Star supports various AI providers including local LLMs (via Ollama), Azure OpenAI, Anthropic's Claude, and Perplexity. The recommended way to configure these is through AI presets, which let you set up and easily switch between different providers and models.
...
### Can I use WSH outside of Star?

`wsh` is an internal CLI for extending control over Star to the command line, you can learn more about it [here](./wsh). To prevent misuse by other applications, `wsh` requires an access token provided by Star to work and will not function outside of the app.

## Why does Star warn me about ARM64 translation when it launches?
...
To get the best experience using Star, it is recommended that you uninstall Star and reinstall the version that is natively compiled for your computer. You can find the right version by consulting our [Installation Instructions](./gettingstarted#installation).

You can disable this warning by setting `app:dismissarchitecturewarning=true` in [your configurations](./config.mdx).

## How do I join the beta builds of Star?

Star publishes to two channels, `latest` and `beta`. If you've installed the app for macOS, Windows, or Linux via DEB or RPM, you can set the following configurations in your `settings.json` (see [Configuration](./config) for more info):
...
```sh
sudo snap install starterm --classic --beta
```
