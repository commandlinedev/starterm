...
import (
	...
	"github.com/commandlinedev/starterm/pkg/util/colprint"
...
const (
	...
	StarFileScheme = "starfile"
	StarFilePrefix = "starfile://"
...
Supported URI schemes:
  wsh:
    ...
    for file streaming to Star and other remotes.
    ...
    If a profile is provided, it must be defined in "profiles.json" in the Star
  ...
  s3:
    ...
    files, or in "profiles.json" in the Star configuration directory.
  ...
  starfile:
    Used to retrieve blockfiles from the internal Star filesystem.
...
var fileInfoCmd = &cobra.Command{
	Use:     "info [uri]",
	Short:   "show star file information",
...
var fileWriteCmd = &cobra.Command{
	...
	Example: "  echo 'hello' | wsh file write starfile://block/greeting.txt",
	Args:    cobra.ExactArgs(1),
...
var fileAppendCmd = &cobra.Command{
	...
	Example: "  tail -f log.txt | wsh file append starfile://block/app.log",
	Args:    cobra.ExactArgs(1),
...
func getTargetPath(src, dst string) (string, error) {
	var srcBase string
	if strings.HasPrefix(src, StarFilePrefix) {
		srcBase = path.Base(src)
	} else {
		srcBase = filepath.Base(src)
	}

	if strings.HasPrefix(dst, StarFilePrefix) {
		// For starfile URLs
