...
var starpathCmd = &cobra.Command{
	Use:     "starpath {config|data|log}",
	Short:   "Get paths to various starterm files and directories",
	RunE:    starpathRun,
	PreRunE: preRunSetupRpcClient,
}

func init() {
	starpathCmd.Flags().BoolP("open", "o", false, "Open the path in a new block")
	starpathCmd.Flags().BoolP("open-external", "O", false, "Open the path in the default external application")
	starpathCmd.Flags().BoolP("tail", "t", false, "Tail the last 100 lines of the log")
	rootCmd.AddCommand(starpathCmd)
}

func starpathRun(cmd *cobra.Command, args []string) (rtnErr error) {
	defer func() {
		sendActivity("starpath", rtnErr == nil)
	}()

	if len(args) == 0 {
		OutputHelpMessage(cmd)
		return fmt.Errorf("no arguments. wsh starpath requires a type argument (config, data, or log)")
	}
	if len(args) > 1 {
		OutputHelpMessage(cmd)
		return fmt.Errorf("too many arguments. wsh starpath requires exactly one argument")
	}
