...
func (AnthropicBackend) StreamCompletion(ctx context.Context, request wshrpc.StarAIStreamRequest) chan wshrpc.RespOrErrorUnion[wshrpc.StarAIPacketType] {
	...
	go func() {
		...
		for {
			...
			case "message_delta":
				...
				if event.Usage != nil {
					pk := MakeStarAIPacket()
					pk.Usage = &wshrpc.StarAIUsageType{
