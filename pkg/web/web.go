...
import (
	...
	"github.com/commandlinedev/starterm/pkg/docsite"
	...
	"github.com/commandlinedev/starterm/pkg/schema"
...
const (
	...
	StarZoneFileInfoHeaderKey = "X-ZoneFileInfo"
)
...
func handleStarFile(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set(StarZoneFileInfoHeaderKey, base64.StdEncoding.EncodeToString(jsonFileBArr))
...
func handleLocalStreamFile(w http.ResponseWriter, r *http.Request, path string, no404 bool) {
	if no404 {
		...
		path, err := starbase.ExpandHomeDir(path)
		if err == nil {
...
func MakeUnixListener() (net.Listener, error) {
	serverAddr := starbase.GetDomainSocketName()
...
func RunWebServer(listener net.Listener) {
	gr := mux.NewRouter()
	gr.HandleFunc("/star/stream-local-file", WebFnWrap(WebFnOpts{AllowCaching: true}, handleStreamLocalFile))
	gr.HandleFunc("/star/stream-file", WebFnWrap(WebFnOpts{AllowCaching: true}, handleStreamFile))
	gr.PathPrefix("/star/stream-file/").HandlerFunc(WebFnWrap(WebFnOpts{AllowCaching: true}, handleStreamFile))
	gr.HandleFunc("/star/file", WebFnWrap(WebFnOpts{AllowCaching: false}, handleStarFile))
	gr.HandleFunc("/star/service", WebFnWrap(WebFnOpts{JsonErrors: true}, handleService))
