...
type UpdatesRtnType = []StarObjUpdate
...
type StarObjUpdate struct {
	UpdateType string  `json:"updatetype"`
	OType      string  `json:"otype"`
	OID        string  `json:"oid"`
	Obj        StarObj `json:"obj,omitempty"`
}

func (update StarObjUpdate) MarshalJSON() ([]byte, error) {
...
func MakeUpdate(obj StarObj) StarObjUpdate {
	return StarObjUpdate{
...
func MakeUpdates(objs []StarObj) []StarObjUpdate {
	rtn := make([]StarObjUpdate, 0, len(objs))
...
func (update *StarObjUpdate) UnmarshalJSON(data []byte) error {
	...
	if _, found := objMap["updatetype"]; !found {
		return fmt.Errorf("missing updatetype (in StarObjUpdate)")
	}
	update.UpdateType, ok1 = objMap["updatetype"].(string)
	if !ok1 {
		return fmt.Errorf("in StarObjUpdate bad updatetype type %T", objMap["updatetype"])
	}
	if _, found := objMap["otype"]; !found {
		return fmt.Errorf("missing otype (in StarObjUpdate)")
	}
	update.OType, ok2 = objMap["otype"].(string)
	if !ok2 {
		return fmt.Errorf("in StarObjUpdate bad otype type %T", objMap["otype"])
	}
	if _, found := objMap["oid"]; !found {
		return fmt.Errorf("missing oid (in StarObjUpdate)")
	}
	update.OID, ok3 = objMap["oid"].(string)
	if !ok3 {
		return fmt.Errorf("in StarObjUpdate bad oid type %T", objMap["oid"])
	}
	if _, found := objMap["obj"]; found {
		objMap, ok := objMap["obj"].(map[string]any)
		if !ok {
			return fmt.Errorf("in StarObjUpdate bad obj type %T", objMap["obj"])
		}
		starObj, err := FromJsonMap(objMap)
		if err != nil {
			return fmt.Errorf("in StarObjUpdate error decoding obj: %w", err)
		}
		update.Obj = starObj
	}
	return nil
...
func AllStarObjTypes() []reflect.Type {
	return []reflect.Type{
