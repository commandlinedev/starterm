...
var starObjUpdateKey = struct{}{}

type contextUpdatesType struct {
	UpdatesStack []map[ORef]StarObjUpdate
}
...
func ContextWithUpdates(ctx context.Context) context.Context {
	updatesVal := ctx.Value(starObjUpdateKey)
	if updatesVal != nil {
		return ctx
	}
	return context.WithValue(ctx, starObjUpdateKey, &contextUpdatesType{
		UpdatesStack: []map[ORef]StarObjUpdate{make(map[ORef]StarObjUpdate)},
	})
}

func ContextGetUpdates(ctx context.Context) map[ORef]StarObjUpdate {
	updatesVal := ctx.Value(starObjUpdateKey)
	if updatesVal == nil {
		return nil
	}
	...
	rtn := make(map[ORef]StarObjUpdate)
...
func ContextGetUpdate(ctx context.Context, oref ORef) *StarObjUpdate {
	updatesVal := ctx.Value(starObjUpdateKey)
	if updatesVal == nil {
		return nil
	}
...
func ContextAddUpdate(ctx context.Context, update StarObjUpdate) {
	updatesVal := ctx.Value(starObjUpdateKey)
	if updatesVal == nil {
		return
	}
...
func ContextUpdatesBeginTx(ctx context.Context) context.Context {
	...
	updates := updatesVal.(*contextUpdatesType)
	updates.UpdatesStack = append(updates.UpdatesStack, make(map[ORef]StarObjUpdate))
	return ctx
}
