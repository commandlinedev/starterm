...
type Client interface {
	SendEvent(routeId string, event StarEvent)
}
...
func (b *BrokerType) ReadEventHistory(eventType string, scope string, maxItems int) []*StarEvent {
	if maxItems <= 0 {
		return nil
	}
	b.Lock.Lock()
	...
	// return new arr
	rtn := make([]*StarEvent, maxItems)
	copy(rtn, pe.Events[len(pe.Events)-maxItems:])
	return rtn
}

func (b *BrokerType) persistEvent(event StarEvent) {
	if event.Persist <= 0 {
		return
	}
	...
	for scope := range scopeMap {
		...
		if pe == nil {
			pe = &persistEventWrap{
				ArrTotalAdds: 0,
				Events:       make([]*StarEvent, 0, event.Persist),
			}
			b.PersistMap[key] = pe
		}
		...
		if pe.ArrTotalAdds > ReMakeArrThreshold {
			pe.Events = append([]*StarEvent{}, pe.Events...)
			pe.ArrTotalAdds = len(pe.Events)
		}
	}
}

func (b *BrokerType) Publish(event StarEvent) {
...
func (b *BrokerType) SendUpdateEvents(updates starobj.UpdatesRtnType) {
	for _, update := range updates {
		b.Publish(StarEvent{
			Event:  Event_StarObjUpdate,
			Scopes: []string{starobj.MakeORef(update.OType, update.OID).String()},
			Data:   update,
		})
	}
}

func (b *BrokerType) getMatchingRouteIds(event StarEvent) []string {
	b.Lock.Lock()
	defer b.Lock.Unlock()
