...
import (
	...
	"github.com/commandlinedev/starterm/pkg/util/pamparse"
...
func StartWslShellProcNoWsh(ctx context.Context, termSize starobj.TermSize, cmdStr string, cmdOpts CommandOptsType, conn *wslconn.WslConn) (*ShellProc, error) {
...
func StartWslShellProc(ctx context.Context, termSize starobj.TermSize, cmdStr string, cmdOpts CommandOptsType, conn *wslconn.WslConn) (*ShellProc, error) {
	client := conn.GetClient()
	...
	if cmdStr == "" {
		...
		if shellType == shellutil.ShellType_bash {
			...
			bashPath := fmt.Sprintf("~/.starterm/%s/.bashrc", shellutil.BashIntegrationDir)
			shellOpts = append(shellOpts, "--rcfile", bashPath)
		} else if shellType == shellutil.ShellType_fish {
			if cmdOpts.Login {
				shellOpts = append(shellOpts, "-l")
			}
			// source the star.fish file
			starFishPath := fmt.Sprintf("~/.starterm/%s/star.fish", shellutil.FishIntegrationDir)
			carg := fmt.Sprintf(`"source %s"`, starFishPath)
			shellOpts = append(shellOpts, "-C", carg)
		} else if shellType == shellutil.ShellType_pwsh {
			pwshPath := fmt.Sprintf("~/.starterm/%s/starpwsh.ps1", shellutil.PwshIntegrationDir)
	...
	if shellType == shellutil.ShellType_zsh {
		zshDir := fmt.Sprintf("~/.starterm/%s", shellutil.ZshIntegrationDir)
	...
	if err != nil {
		...
		cmdCombined = fmt.Sprintf(`%s=%s %s`, starbase.StarSwapTokenVarName, packedToken, cmdCombined)
	}
	jwtToken := cmdOpts.SwapToken.Env[starbase.StarJwtTokenVarName]
	if jwtToken != "" {
		cmdCombined = fmt.Sprintf(`%s=%s %s`, starbase.StarJwtTokenVarName, jwtToken, cmdCombined)
	}
...
func StartRemoteShellProcNoWsh(ctx context.Context, termSize starobj.TermSize, cmdStr string, cmdOpts CommandOptsType, conn *conncontroller.SSHConn) (*ShellProc, error) {
	client := conn.GetClient()
...
func StartRemoteShellProc(ctx context.Context, logCtx context.Context, termSize starobj.TermSize, cmdStr string, cmdOpts CommandOptsType, conn *conncontroller.SSHConn) (*ShellProc, error) {
	client := conn.GetClient()
	connRoute := wshutil.MakeConnectionRouteId(conn.GetName())
	rpcClient := wshclient.GetBareRpcClient()
	remoteInfo, err := wshclient.RemoteGetInfoCommand(rpcClient, &wshrpc.RpcOpts{Route: connRoute, Timeout: 2000})
...
func StartLocalShellProc(logCtx context.Context, termSize starobj.TermSize, cmdStr string, cmdOpts CommandOptsType) (*ShellProc, error) {
	...
	if cmdStr == "" {
		if shellType == shellutil.ShellType_bash {
			...
			starFishPath := shellutil.GetLocalStarFishFilePath()
			carg := fmt.Sprintf("source %s", shellutil.HardQuoteFish(starFishPath))
			shellOpts = append(shellOpts, "-C", carg)
		} else if shellType == shellutil.ShellType_pwsh {
			shellOpts = append(shellOpts, "-ExecutionPolicy", "Bypass", "-NoExit", "-File", shellutil.GetLocalStarPowershellEnv())
		} else {
			if cmdOpts.Login {
				shellOpts = append(shellOpts, "-l")
			}
	...
	if err != nil {
		...
		shellutil.UpdateCmdEnv(ecmd, map[string]string{starbase.StarSwapTokenVarName: packedToken})
	}
	...
	*/
	...
	if cwdErr := checkCwd(ecmd.Dir); cwdErr != nil {
		ecmd.Dir = starbase.GetHomeDir()
	}
	envToAdd := shellutil.StarshellLocalEnvVars(shellutil.DefaultTermType)
	if os.Getenv("LANG") == "" {
		envToAdd["LANG"] = starbase.DetermineLang()
	}
...
func RunSimpleCmdInPty(ecmd *exec.Cmd, termSize starobj.TermSize) ([]byte, error) {
	ecmd.Env = os.Environ()
	shellutil.UpdateCmdEnv(ecmd, shellutil.StarshellLocalEnvVars(shellutil.DefaultTermType))
	if termSize.Rows == 0 || termSize.Cols == 0 {
		termSize.Rows = shellutil.DefaultTermRows
		termSize.Cols = shellutil.DefaultTermCols
	}
...
func tryGetPamEnvVars() map[string]string {
	...
	envVars3, err := pamparse.ParseEnvironmentConfFile(starbase.ExpandHomeDirSafe(userEnvironmentPath), pamParseOpts)
	if err != nil {
