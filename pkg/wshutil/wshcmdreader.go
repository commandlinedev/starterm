...
func MakePtyBuffer(oscPrefix string, input io.Reader, messageCh chan []byte) *PtyBuffer {
	if len(oscPrefix) != StarOSCPrefixLen {
...
func (b *PtyBuffer) processStarEscSeq(escSeq []byte) {
	b.MessageCh <- escSeq
}
	...
	for _, ch := range data {
		if b.EscMode == Mode_StarEsc {
			if ch == ESC {
				...
				// terminates the escpae sequence (is a valid Star OSC command)
				b.EscMode = Mode_Normal
				starEscSeq := b.EscSeqBuf[StarOSCPrefixLen:]
				b.EscSeqBuf = nil
				b.processStarEscSeq(starEscSeq)
			} else {
		...
		if b.EscMode == Mode_Esc {
			if ch == ESC || ch == BEL || ch == ST {
				// these all terminate the escape sequence (invalid, not a Star OSC)
				b.EscMode = Mode_Normal
				outputBuf = append(outputBuf, b.EscSeqBuf...)
				outputBuf = append(outputBuf, ch)
				b.EscSeqBuf = nil
			...
			if ch != b.OSCPrefix[len(b.EscSeqBuf)] {
				// this is not a Star OSC sequence, just an escape sequence
				b.EscMode = Mode_Normal
				outputBuf = append(outputBuf, b.EscSeqBuf...)
				outputBuf = append(outputBuf, ch)
				b.EscSeqBuf = nil
			...
			// we're still building what could be a Star OSC sequence
			b.EscSeqBuf = append(b.EscSeqBuf, ch)
			// check to see if we have a full Star OSC prefix
			if len(b.EscSeqBuf) == len(b.OSCPrefix) {
				b.EscMode = Mode_StarEsc
			}
			continue
		}
		// Mode_Normal
