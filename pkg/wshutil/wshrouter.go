...
func (router *WshRouter) SendEvent(routeId string, event wps.StarEvent) {
	defer func() {
...
func (router *WshRouter) UnregisterRoute(routeId string) {
	...
	go func() {
		...
		wps.Broker.Publish(wps.StarEvent{Event: wps.Event_RouteGone, Scopes: []string{routeId}})
	}()
}
