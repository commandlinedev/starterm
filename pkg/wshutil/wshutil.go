...
const StarOSC = "23198"
const StarServerOSC = "23199"
const StarOSCPrefixLen = 5 + 3 // \x1b] + StarOSC + ; + \x07

const StarOSCPrefix = "\x1b]" + StarOSC + ";"
const StarServerOSCPrefix = "\x1b]" + StarServerOSC + ";"
...
const StarJwtTokenVarName = starbase.StarJwtTokenVarName
...
func EncodeStarOSCBytes(oscNum string, barr []byte) ([]byte, error) {
	if len(oscNum) != 5 {
...
func EncodeStarOSCMessageEx(oscNum string, msg *RpcMessage) ([]byte, error) {
	if msg == nil {
	...
	return EncodeStarOSCBytes(oscNum, barr)
}
...
func SetupTerminalRpcClient(serverImpl ServerImpl, debugStr string) (*WshRpc, io.Reader) {
	...
	ptyBuf := MakePtyBuffer(StarServerOSCPrefix, os.Stdin, messageCh)
	...
	go func() {
		...
		for msg := range outputCh {
			barr, err := EncodeStarOSCBytes(StarOSC, msg)
			if err != nil {
...
func MakeClientJWTToken(rpcCtx wshrpc.RpcContext, sockName string) (string, error) {
	...
	tokenStr, err := token.SignedString([]byte(starbase.JwtSecret))
	if err != nil {
...
func ValidateAndExtractRpcContextFromToken(tokenStr string) (*wshrpc.RpcContext, error) {
	...
	token, err := parser.Parse(tokenStr, func(token *jwt.Token) (interface{}, error) {
		return []byte(starbase.JwtSecret), nil
	})
	if err != nil {
...
func ExtractUnverifiedSocketName(tokenStr string) (string, error) {
	...
	sockName = starbase.ExpandHomeDirSafe(sockName)
	return sockName, nil
}
...
func GetInfo() wshrpc.RemoteInfo {
	return wshrpc.RemoteInfo{
		...
		ClientVersion: starbase.StarVersion,
		Shell:         getShell(),
	}
...
func InstallRcFiles() error {
	home := starbase.GetHomeDir()
	starDir := filepath.Join(home, starbase.RemoteStarHomeDirName)
	wshBinDir := filepath.Join(starDir, starbase.RemoteWshBinDirName)
	return shellutil.InitRcFiles(starDir, wshBinDir)
}
