...
const (
	...
	Command_StreamStarAi         = "streamstarai"
...
type WshRpcInterface interface {
	...
	GetMetaCommand(ctx context.Context, data CommandGetMetaData) (starobj.MetaMapType, error)
	...
	CreateBlockCommand(ctx context.Context, data CommandCreateBlockData) (starobj.ORef, error)
	CreateSubBlockCommand(ctx context.Context, data CommandCreateSubBlockData) (starobj.ORef, error)
	...
	EventPublishCommand(ctx context.Context, data wps.StarEvent) error
	EventSubCommand(ctx context.Context, data wps.SubscriptionRequest) error
	EventUnsubCommand(ctx context.Context, data string) error
	EventUnsubAllCommand(ctx context.Context) error
	EventReadHistoryCommand(ctx context.Context, data CommandEventReadHistoryData) ([]*wps.StarEvent, error)
	StreamTestCommand(ctx context.Context) chan RespOrErrorUnion[int]
	StreamStarAiCommand(ctx context.Context, request StarAIStreamRequest) chan RespOrErrorUnion[StarAIPacketType]
	...
	GetTabCommand(ctx context.Context, tabId string) (*starobj.Tab, error)
	...
	EventRecvCommand(ctx context.Context, data wps.StarEvent) error
	...
	NotifyCommand(ctx context.Context, notificationOptions StarNotificationOptions) error
	...
	// terminal
	VDomCreateContextCommand(ctx context.Context, data vdom.VDomCreateContext) (*starobj.ORef, error)
	...
	for i := 0; i < dataVal.NumField(); i++ {
		...
		case "BlockORef":
			if rpcContext.BlockId != "" {
				field.Set(reflect.ValueOf(starobj.MakeORef(starobj.OType_Block, rpcContext.BlockId)))
			}
		default:
...
type CommandMessageData struct {
	ORef    starobj.ORef `json:"oref" wshcontext:"BlockORef"`
...
type CommandSetMetaData struct {
	ORef starobj.ORef        `json:"oref" wshcontext:"BlockORef"`
	Meta starobj.MetaMapType `json:"meta"`
}
...
type StarAIStreamRequest struct {
	ClientId string                    `json:"clientid,omitempty"`
	Opts     *StarAIOptsType           `json:"opts"`
	Prompt   []StarAIPromptMessageType `json:"prompt"`
}

type StarAIPromptMessageType struct {
	Role    string `json:"role"`
	Content string `json:"content"`
	Name    string `json:"name,omitempty"`
}

type StarAIOptsType struct {
...
type StarAIPacketType struct {
	Type         string           `json:"type"`
	Model        string           `json:"model,omitempty"`
	Created      int64            `json:"created,omitempty"`
	FinishReason string           `json:"finish_reason,omitempty"`
	Usage        *StarAIUsageType `json:"usage,omitempty"`
	Index        int              `json:"index,omitempty"`
	Text         string           `json:"text,omitempty"`
	Error        string           `json:"error,omitempty"`
}

type StarAIUsageType struct {
...
func (m *MetaSettingsType) UnmarshalJSON(data []byte) error {
	var metaMap starobj.MetaMapType
...
type ConnConfigRequest struct {
	Host        string              `json:"host"`
	MetaMapType starobj.MetaMapType `json:"metamaptype"`
}
...
type BlockInfoData struct {
	...
	WorkspaceId string         `json:"workspaceid"`
	Block       *starobj.Block `json:"block"`
	Files       []*FileInfo    `json:"files"`
}

type StarNotificationOptions struct {
...
type WorkspaceInfoData struct {
	...
	WorkspaceData *starobj.Workspace `json:"workspacedata"`
}
