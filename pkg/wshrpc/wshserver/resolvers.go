...
func parseSimpleId(simpleId string) (discriminator string, value string, err error) {
	...
	if _, err := starobj.ParseORef(simpleId); err == nil {
...
func resolveThis(ctx context.Context, data wshrpc.CommandResolveIdsData, value string) (*starobj.ORef, error) {
	if data.BlockId == "" {
		return nil, fmt.Errorf("no blockid in request")
	}

	if value == SimpleId_This || value == SimpleId_Block {
		return &starobj.ORef{OType: starobj.OType_Block, OID: data.BlockId}, nil
	}
	if value == SimpleId_Tab {
		...
		return &starobj.ORef{OType: starobj.OType_Tab, OID: tabId}, nil
	}
	if value == SimpleId_Ws || value == SimpleId_Workspace {
		...
		return &starobj.ORef{OType: starobj.OType_Workspace, OID: wsId}, nil
	}
	if value == SimpleId_Client || value == SimpleId_Global {
		...
		return &starobj.ORef{OType: starobj.OType_Client, OID: client.OID}, nil
	}
	if value == SimpleId_Temp {
		...
		return &starobj.ORef{OType: "temp", OID: client.TempOID}, nil
	}
	return nil, fmt.Errorf("invalid value for 'this' resolver: %s", value)
}

func resolveORef(_ context.Context, value string) (*starobj.ORef, error) {
	parsedORef, err := starobj.ParseORef(value)
	if err != nil {
...
func resolveTabNum(ctx context.Context, data wshrpc.CommandResolveIdsData, value string) (*starobj.ORef, error) {
	...
	ws, err := wstore.DBMustGet[*starobj.Workspace](ctx, wsId)
	if err != nil {
	...
	return &starobj.ORef{OType: starobj.OType_Tab, OID: resolvedTabId}, nil
}

func resolveBlock(ctx context.Context, data wshrpc.CommandResolveIdsData, value string) (*starobj.ORef, error) {
	...
	layout, err := wstore.DBGet[*starobj.LayoutState](ctx, tab.LayoutState)
	if err != nil {
	...
	return &starobj.ORef{OType: starobj.OType_Block, OID: leafEntry.BlockId}, nil
}

func resolveView(ctx context.Context, data wshrpc.CommandResolveIdsData, value string) (*starobj.ORef, error) {
	...
	layout, err := wstore.DBMustGet[*starobj.LayoutState](ctx, tab.LayoutState)
	if err != nil {
	...
	for _, leaf := range *layout.LeafOrder {
		...
		leafBlock, err := wstore.DBMustGet[*starobj.Block](ctx, leafBlockId)
		if err != nil {
			continue
		}
		if leafBlock.Meta.GetString("view", "") == viewType {
			...
			if count == instanceNum {
				return &starobj.ORef{OType: starobj.OType_Block, OID: leaf.BlockId}, nil
			}
		}
	}
...
func resolveUUID(ctx context.Context, value string) (*starobj.ORef, error) {
	return wstore.DBResolveEasyOID(ctx, value)
}

// Main resolver function
func resolveSimpleId(ctx context.Context, data wshrpc.CommandResolveIdsData, simpleId string) (*starobj.ORef, error) {
