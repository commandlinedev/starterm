...
func UpdateActivity(ctx context.Context, update wshrpc.ActivityUpdate) error {
	...
	txErr := wstore.WithTx(ctx, func(tx *wstore.TxWrap) error {
		...
		if !found {
			...
			tx.Exec(query, dayStr, tdata, tzName, tzOffset, starbase.StarVersion, starbase.ClientArch(), starbase.BuildTime, starbase.UnameKernelRelease())
		}
		...
		query = `UPDATE db_activity
                 SET tdata = ?,
                     ...
                     buildtime = ?
                 WHERE day = ?`
		tx.Exec(query, tdata, starbase.StarVersion, starbase.BuildTime, dayStr)
		return nil
	})
	if txErr != nil {
		return txErr
