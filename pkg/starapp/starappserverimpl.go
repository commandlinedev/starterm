...
type StarAppServerImpl struct {
	Client  *Client
	BlockId string
}

func (*StarAppServerImpl) WshServerImpl() {}

func (impl *StarAppServerImpl) VDomRenderCommand(ctx context.Context, feUpdate vdom.VDomFrontendUpdate) chan wshrpc.RespOrErrorUnion[*vdom.VDomBackendUpdate] {
	...
	for _, event := range feUpdate.Events {
		if event.GlobalEventType != "" {
			...
			impl.Client.Root.Event(event.StarId, event.EventType, event)
		}
	}
	// update refs
...
func (impl *StarAppServerImpl) VDomUrlRequestCommand(ctx context.Context, data wshrpc.VDomUrlRequestData) chan wshrpc.RespOrErrorUnion[wshrpc.VDomUrlRequestResponse] {
	...
	go func() {
		...
		if httpReq.URL.Path == "/star/global.css" && impl.Client.GlobalStylesOption != nil {
