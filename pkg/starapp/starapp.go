...
func (client *Client) Connect() error {
	...
	if jwtToken == "" {
		return fmt.Errorf("no %s env var set", wshutil.StarJwtTokenVarName)
	}
	...
	client.ServerImpl = &StarAppServerImpl{BlockId: client.RpcContext.BlockId, Client: client}
...
func (c *Client) CreateVDomContext(target *vdom.VDomTarget) error {
	...
	c.RpcClient.EventListener.On("blockclose", func(event *wps.StarEvent) {
...
func makeNullVDom() *vdom.VDomElem {
	return &vdom.VDomElem{StarId: uuid.New().String(), Tag: vdom.StarNullTag}
}
...
func determineMimeType(option FileHandlerOption) (string, []byte) {
	...
	if option.FilePath != "" {
		filePath := starbase.ExpandHomeDirSafe(option.FilePath)
