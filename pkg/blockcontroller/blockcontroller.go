...
type BlockInputUnion struct {
	...
	TermSize  *starobj.TermSize `json:"termsize,omitempty"`
}
...
func (bc *BlockController) UpdateControllerAndSendUpdate(updateFn func() bool) {
	...
	if sendUpdate {
		...
		wps.Broker.Publish(wps.StarEvent{
			...
			Scopes: []string{
				starobj.MakeORef(starobj.OType_Tab, bc.TabId).String(),
				starobj.MakeORef(starobj.OType_Block, bc.BlockId).String(),
			},
			Data: rtStatus,
		})
	}
...
func HandleTruncateBlockFile(blockId string) error {
	...
	defer cancelFn()
	err := filestore.WFS.WriteFile(ctx, blockId, starbase.BlockFile_Term, nil)
	...
	err = filestore.WFS.DeleteFile(ctx, blockId, starbase.BlockFile_Cache)
	if err == fs.ErrNotExist {
		err = nil
	}
	if err != nil {
	...
	wps.Broker.Publish(wps.StarEvent{
		...
		Scopes: []string{starobj.MakeORef(starobj.OType_Block, blockId).String()},
		Data: &wps.WSFileEventData{
			ZoneId:   blockId,
			FileName: starbase.BlockFile_Term,
...
func (bc *BlockController) resetTerminalState(logCtx context.Context) {
	ctx, cancelFn := context.WithTimeout(context.Background(), DefaultTimeout)
	defer cancelFn()
	wfile, statErr := filestore.WFS.Stat(ctx, bc.BlockId, starbase.BlockFile_Term)
	...
	buf.WriteString("\r\n\r\n")
	err := HandleAppendBlockFile(bc.BlockId, starbase.BlockFile_Term, buf.Bytes())
	if err != nil {
...
func getCustomInitScriptKeyCascade(shellType string) []string {
	if shellType == "bash" {
		return []string{starobj.MetaKey_CmdInitScriptBash, starobj.MetaKey_CmdInitScriptSh, starobj.MetaKey_CmdInitScript}
	}
	if shellType == "zsh" {
		return []string{starobj.MetaKey_CmdInitScriptZsh, starobj.MetaKey_CmdInitScriptSh, starobj.MetaKey_CmdInitScript}
	}
	if shellType == "pwsh" {
		return []string{starobj.MetaKey_CmdInitScriptPwsh, starobj.MetaKey_CmdInitScript}
	}
	if shellType == "fish" {
		return []string{starobj.MetaKey_CmdInitScriptFish, starobj.MetaKey_CmdInitScript}
	}
	return []string{starobj.MetaKey_CmdInitScript}
}

func getCustomInitScript(logCtx context.Context, meta starobj.MetaMapType, connName string, shellType string) string {
	...
	initScriptVal, err := starbase.ExpandHomeDir(initScriptVal)
	if err != nil {
		blocklogger.Infof(logCtx, "[conndebug] cannot expand home dir in Star initscript file: %v\n", err)
		return fmt.Sprintf("echo \"cannot expand home dir in Star initscript file, from key %s\";\n", metaKeyName)
	}
	fileData, err := os.ReadFile(initScriptVal)
	if err != nil {
		blocklogger.Infof(logCtx, "[conndebug] cannot open Star initscript file: %v\n", err)
		return fmt.Sprintf("echo \"cannot open Star initscript file, from key %s\";\n", metaKeyName)
	}
...
func getCustomInitScriptValue(meta starobj.MetaMapType, connName string, shellType string) (string, string) {
	...
	ckMeta := starobj.MetaMapType(connKeywordsMap)
	for _, key := range keys {
...
func resolveEnvMap(blockId string, blockMeta starobj.MetaMapType, connName string) (map[string]string, error) {
	...
	defer cancelFn()
	_, envFileData, err := filestore.WFS.ReadFile(ctx, blockId, starbase.BlockFile_Env)
	if err == fs.ErrNotExist {
		err = nil
	}
	if err != nil {
	...
	cmdEnv := blockMeta.GetStringMap(starobj.MetaKey_CmdEnv, true)
	for k, v := range cmdEnv {
		if v == starobj.MetaMap_DeleteSentinel {
			delete(rtn, k)
			continue
		}
		rtn[k] = v
	}
	connEnv := blockMeta.GetConnectionOverride(connName).GetStringMap(starobj.MetaKey_CmdEnv, true)
...
// for "cmd" type blocks
func createCmdStrAndOpts(blockId string, blockMeta starobj.MetaMapType, connName string) (string, *shellexec.CommandOptsType, error) {
	var cmdStr string
	var cmdOpts shellexec.CommandOptsType
	cmdStr = blockMeta.GetString(starobj.MetaKey_Cmd, "")
	if cmdStr == "" {
		return "", nil, fmt.Errorf("missing cmd in block meta")
	}
	cmdOpts.Cwd = blockMeta.GetString(starobj.MetaKey_CmdCwd, "")
	if cmdOpts.Cwd != "" {
		cwdPath, err := starbase.ExpandHomeDir(cmdOpts.Cwd)
		if err != nil {
			return "", nil, err
		}
		cmdOpts.Cwd = cwdPath
	}
	useShell := blockMeta.GetBool(starobj.MetaKey_CmdShell, true)
	if !useShell {
		if strings.Contains(cmdStr, " ") {
			return "", nil, fmt.Errorf("cmd should not have spaces if cmd:shell is false (use cmd:args)")
		}
		cmdArgs := blockMeta.GetStringList(starobj.MetaKey_CmdArgs)
...
func (bc *BlockController) DoRunShellCommand(logCtx context.Context, rc *RunShellOpts, blockMeta starobj.MetaMapType) error {
...
func (bc *BlockController) makeSwapToken(ctx context.Context, logCtx context.Context, blockMeta starobj.MetaMapType, remoteName string, shellType string) *shellutil.TokenSwapEntry {
	token := &shellutil.TokenSwapEntry{
		Token: uuid.New().String(),
		Env:   make(map[string]string),
		Exp:   time.Now().Add(5 * time.Minute),
	}
	token.Env["TERM_PROGRAM"] = "starterm"
	token.Env["STARTERM_BLOCKID"] = bc.BlockId
	token.Env["STARTERM_VERSION"] = starbase.StarVersion
	token.Env["STARTERM"] = "1"
	tabId, err := wstore.DBFindTabForBlockId(ctx, bc.BlockId)
	if err != nil {
		log.Printf("error finding tab for block: %v\n", err)
	} else {
		token.Env["STARTERM_TABID"] = tabId
	}
	if tabId != "" {
		...
		if err != nil {
			...
			token.Env["STARTERM_WORKSPACEID"] = wsId
		}
	}
	...
	if err != nil {
		...
		token.Env["STARTERM_CLIENTID"] = clientData.OID
	}
	token.Env["STARTERM_CONN"] = remoteName
...
func getLocalShellPath(blockMeta starobj.MetaMapType) string {
	shellPath := blockMeta.GetString(starobj.MetaKey_TermLocalShellPath, "")
	if shellPath != "" {
		return shellPath
	}
...
func getLocalShellOpts(blockMeta starobj.MetaMapType) []string {
	if blockMeta.HasKey(starobj.MetaKey_TermLocalShellOpts) {
...
func (union *ConnUnion) getRemoteInfoAndShellType(blockMeta starobj.MetaMapType) error {
	if !union.WshEnabled {
		return nil
	}
...
func (bc *BlockController) getConnUnion(logCtx context.Context, remoteName string, blockMeta starobj.MetaMapType) (ConnUnion, error) {
	rtn := ConnUnion{ConnName: remoteName}
	wshEnabled := !blockMeta.GetBool(starobj.MetaKey_CmdNoWsh, false)
...
func (bc *BlockController) setupAndStartShellProcess(logCtx context.Context, rc *RunShellOpts, blockMeta starobj.MetaMapType) (*shellexec.ShellProc, error) {
	// create a circular blockfile for the output
	ctx, cancelFn := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancelFn()
	fsErr := filestore.WFS.MakeFile(ctx, bc.BlockId, starbase.BlockFile_Term, nil, wshrpc.FileOpts{MaxSize: DefaultTermMaxFileSize, Circular: true})
	...
	remoteName := blockMeta.GetString(starobj.MetaKey_Connection, "")
	...
	if connUnion.ConnType == ConnType_Wsl {
		...
		if !connUnion.WshEnabled {
			...
			swapToken.Env[wshutil.StarJwtTokenVarName] = jwtStr
		...
		if connUnion.WshEnabled {
			sockName := starbase.GetDomainSocketName()
...
func (bc *BlockController) getBlockData_noErr() *starobj.Block {
	ctx, cancelFn := context.WithTimeout(context.Background(), DefaultTimeout)
	defer cancelFn()
	blockData, err := wstore.DBGet[*starobj.Block](ctx, bc.BlockId)
	if err != nil {
...
func (bc *BlockController) manageRunningShellProcess(shellProc *shellexec.ShellProc, rc *RunShellOpts, blockMeta starobj.MetaMapType) error {
	...
	ptyBuffer := wshutil.MakePtyBuffer(wshutil.StarOSCPrefix, shellProc.Cmd, wshProxy.FromRemoteCh)
	go func() {
		...
		defer func() {
			...
			if blockData != nil && blockData.Meta.GetString(starobj.MetaKey_Controller, "") == BlockController_Cmd {
				termMsg := fmt.Sprintf("\r\nprocess finished with exit code = %d\r\n\r\n", exitCode)
				HandleAppendBlockFile(bc.BlockId, starbase.BlockFile_Term, []byte(termMsg))
			}
		...
		for {
			...
			if nr > 0 {
				err := HandleAppendBlockFile(bc.BlockId, starbase.BlockFile_Term, buf[:nr])
				if err != nil {
	...
	go func() {
		...
		for msg := range wshProxy.ToRemoteCh {
			encodedMsg, err := wshutil.EncodeStarOSCBytes(wshutil.StarServerOSC, msg)
			if err != nil {
...
func updateTermSize(shellProc *shellexec.ShellProc, blockId string, termSize starobj.TermSize) {
...
func checkCloseOnExit(blockId string, exitCode int) {
	...
	closeOnExit := blockData.Meta.GetBool(starobj.MetaKey_CmdCloseOnExit, false)
	closeOnExitForce := blockData.Meta.GetBool(starobj.MetaKey_CmdCloseOnExitForce, false)
	if !closeOnExitForce && !(closeOnExit && exitCode == 0) {
		return
	}
	delayMs := blockData.Meta.GetFloat(starobj.MetaKey_CmdCloseOnExitDelay, 2000)
	if delayMs < 0 {
		delayMs = 0
	}
...
func getTermSize(bdata *starobj.Block) starobj.TermSize {
	if bdata.RuntimeOpts != nil {
		return bdata.RuntimeOpts.TermSize
	} else {
		return starobj.TermSize{
			Rows: 25,
			Cols: 80,
		}
	}
...
func setTermSizeInDB(blockId string, termSize starobj.TermSize) error {
	ctx, cancelFn := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancelFn()
	ctx = starobj.ContextWithUpdates(ctx)
	bdata, err := wstore.DBMustGet[*starobj.Block](ctx, blockId)
	if err != nil {
		return fmt.Errorf("error getting block data: %v", err)
	}
	if bdata.RuntimeOpts == nil {
		bdata.RuntimeOpts = &starobj.RuntimeOpts{}
	}
...
func (bc *BlockController) run(logCtx context.Context, bdata *starobj.Block, blockMeta map[string]any, rtOpts *starobj.RuntimeOpts, force bool) {
	...
	controllerName := bdata.Meta.GetString(starobj.MetaKey_Controller, "")
	if controllerName != BlockController_Shell && controllerName != BlockController_Cmd {
		log.Printf("unknown controller %q\n", controllerName)
		return
	}
	runOnce := getBoolFromMeta(blockMeta, starobj.MetaKey_CmdRunOnce, false)
	runOnStart := getBoolFromMeta(blockMeta, starobj.MetaKey_CmdRunOnStart, true)
	if ((runOnStart || runOnce) && curStatus.ShellProcStatus == Status_Init) || force {
		if getBoolFromMeta(blockMeta, starobj.MetaKey_CmdClearOnStart, false) {
		...
		if runOnce {
			...
			metaUpdate := map[string]any{
				starobj.MetaKey_CmdRunOnce:    false,
				starobj.MetaKey_CmdRunOnStart: false,
			}
			err := wstore.UpdateObjectMeta(ctx, starobj.MakeORef(starobj.OType_Block, bc.BlockId), metaUpdate, false)
			if err != nil {
...
func CheckConnStatus(blockId string) error {
	bdata, err := wstore.DBMustGet[*starobj.Block](context.Background(), blockId)
	if err != nil {
		return fmt.Errorf("error getting block: %w", err)
	}
	connName := bdata.Meta.GetString(starobj.MetaKey_Connection, "")
	if connName == "" {
		return nil
	}
...
func ResyncController(ctx context.Context, tabId string, blockId string, rtOpts *starobj.RuntimeOpts, force bool) error {
	if tabId == "" || blockId == "" {
	...
	connName := blockData.Meta.GetString(starobj.MetaKey_Connection, "")
	controllerName := blockData.Meta.GetString(starobj.MetaKey_Controller, "")
...
func startBlockController(ctx context.Context, tabId string, blockId string, rtOpts *starobj.RuntimeOpts, force bool) error {
	blockData, err := wstore.DBMustGet[*starobj.Block](ctx, blockId)
	if err != nil {
		return fmt.Errorf("error getting block: %w", err)
	}
