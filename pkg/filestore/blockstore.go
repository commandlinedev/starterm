...
// the blockstore package implements a write cache for star files
...
func (f StarFile) DataLength() int64 {
	if f.Opts.Circular {
...
func (f StarFile) DataStartIdx() int64 {
...
func (f *StarFile) DeepCopy() *StarFile {
	if f == nil {
		return nil
	}
	newFile := *f
...
func (s *FileStore) Stat(ctx context.Context, zoneId string, name string) (*StarFile, error) {
	return withLockRtn(s, zoneId, name, func(entry *CacheEntry) (*StarFile, error) {
...
func (s *FileStore) ListFiles(ctx context.Context, zoneId string) ([]*StarFile, error) {
...
func metaIncrement(file *StarFile, key string, amount int) int {
	if file.Meta == nil {
...
func (f *StarFile) partIdxAtOffset(offset int64) int {
...
func (file *StarFile) computePartMap(startOffset int64, size int64) map[int]int {
