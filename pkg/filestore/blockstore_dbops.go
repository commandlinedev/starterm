...
func dbInsertFile(ctx context.Context, file *StarFile) error {
	// will fail if file already exists
	return WithTx(ctx, func(tx *TxWrap) error {
		query := "SELECT zoneid FROM db_star_file WHERE zoneid = ? AND name = ?"
		if tx.Exists(query, file.ZoneId, file.Name) {
			return fs.ErrExist
		}
		query = "INSERT INTO db_star_file (zoneid, name, size, createdts, modts, opts, meta) VALUES (?, ?, ?, ?, ?, ?, ?)"
...
func dbDeleteFile(ctx context.Context, zoneId string, name string) error {
	return WithTx(ctx, func(tx *TxWrap) error {
		query := "DELETE FROM db_star_file WHERE zoneid = ? AND name = ?"
...
func dbGetZoneFileNames(ctx context.Context, zoneId string) ([]string, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) ([]string, error) {
		var files []string
		query := "SELECT name FROM db_star_file WHERE zoneid = ?"
		tx.Select(&files, query, zoneId)
		return files, nil
	})
}

func dbGetZoneFile(ctx context.Context, zoneId string, name string) (*StarFile, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) (*StarFile, error) {
		query := "SELECT * FROM db_star_file WHERE zoneid = ? AND name = ?"
		file := dbutil.GetMappable[*StarFile](tx, query, zoneId, name)
		return file, nil
	})
}
...
func dbGetAllZoneIds(ctx context.Context) ([]string, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) ([]string, error) {
		var ids []string
		query := "SELECT DISTINCT zoneid FROM db_star_file"
		tx.Select(&ids, query)
		return ids, nil
	})
}
...
func dbGetZoneFiles(ctx context.Context, zoneId string) ([]*StarFile, error) {
	return WithTxRtn(ctx, func(tx *TxWrap) ([]*StarFile, error) {
		query := "SELECT * FROM db_star_file WHERE zoneid = ?"
		files := dbutil.SelectMappable[*StarFile](tx, query, zoneId)
		return files, nil
	})
}

func dbWriteCacheEntry(ctx context.Context, file *StarFile, dataEntries map[int]*DataCacheEntry, replace bool) error {
	return WithTx(ctx, func(tx *TxWrap) error {
		query := `SELECT zoneid FROM db_star_file WHERE zoneid = ? AND name = ?`
		if !tx.Exists(query, file.ZoneId, file.Name) {
		...
		query = `UPDATE db_star_file SET size = ?, modts = ?, meta = ? WHERE zoneid = ? AND name = ?`
