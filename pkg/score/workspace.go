...
func CreateWorkspace(ctx context.Context, name string, icon string, color string, applyDefaults bool, isInitialLaunch bool) (*starobj.Workspace, error) {
	ws := &starobj.Workspace{
		OID:          uuid.NewString(),
		TabIds:       []string{},
		PinnedTabIds: []string{},
		Name:         "",
...
func UpdateWorkspace(ctx context.Context, workspaceId string, name string, icon string, color string, applyDefaults bool) (*starobj.Workspace, bool, error) {
...
func DeleteWorkspace(ctx context.Context, workspaceId string, force bool) (bool, string, error) {
	...
	workspace, err := wstore.DBMustGet[*starobj.Workspace](ctx, workspaceId)
	...
	err = wstore.DBDelete(ctx, starobj.OType_Workspace, workspaceId)
	if err != nil {
...
func GetWorkspace(ctx context.Context, wsID string) (*starobj.Workspace, error) {
	return wstore.DBMustGet[*starobj.Workspace](ctx, wsID)
}
...
func CreateTab(ctx context.Context, workspaceId string, tabName string, activateTab bool, pinned bool, isInitialLaunch bool) (string, error) {
	...
	if !isInitialLaunch {
		...
		if presetErr != nil {
			...
			tabORef := starobj.ORefFromStarObj(tab)
...
func createTabObj(ctx context.Context, workspaceId string, name string, pinned bool) (*starobj.Tab, error) {
	ws, err := GetWorkspace(ctx, workspaceId)
	if err != nil {
	...
	layoutState := &starobj.LayoutState{
		OID: layoutStateId,
	}
	if pinned {
...
func DeleteTab(ctx context.Context, workspaceId string, tabId string, recursive bool) (string, error) {
	ws, _ := wstore.DBGet[*starobj.Workspace](ctx, workspaceId)
	if ws == nil {
	...
	tab, _ := wstore.DBGet[*starobj.Tab](ctx, tabId)
	if tab == nil {
	...
	wstore.DBUpdate(ctx, ws)
	wstore.DBDelete(ctx, starobj.OType_Tab, tabId)
	wstore.DBDelete(ctx, starobj.OType_LayoutState, tab.LayoutState)
...
func SendActiveTabUpdate(ctx context.Context, workspaceId string, newActiveTabId string) {
	eventbus.SendEventToElectron(eventbus.WSEventType{
		...
		Data:      &starobj.ActiveTabUpdate{WorkspaceId: workspaceId, NewActiveTabId: newActiveTabId},
	})
}
...
func ListWorkspaces(ctx context.Context) (starobj.WorkspaceList, error) {
	workspaces, err := wstore.DBGetAllObjsByType[*starobj.Workspace](ctx, starobj.OType_Workspace)
	if err != nil {
		return nil, err
	}
	...
	windows, err := wstore.DBGetAllObjsByType[*starobj.Window](ctx, starobj.OType_Window)
	if err != nil {
		return nil, err
	}
	...
	for _, workspace := range workspaces {
		...
		wl = append(wl, &starobj.WorkspaceListEntry{
...
func SetIcon(workspaceId string, icon string) error {
	...
	defer cancel()
	ws, e := wstore.DBGet[*starobj.Workspace](ctx, workspaceId)
	if e != nil {
		return e
	}
	if ws == nil {
