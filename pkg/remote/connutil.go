...
func GetClientPlatform(ctx context.Context, shell genconn.ShellClient) (string, string, error) {
	...
	if err := starbase.ValidateWshSupportedArch(os, arch); err != nil {
		return "", "", err
	}
	return os, arch, nil
}
...
func CpWshToRemote(ctx context.Context, client *ssh.Client, clientOs string, clientArch string) error {
	...
	wshLocalPath, err := shellutil.GetLocalWshBinaryPath(starbase.StarVersion, clientOs, clientArch)
	if err != nil {
		return err
	}
	...
	installWords := map[string]string{
		"installDir":  filepath.ToSlash(filepath.Dir(starbase.RemoteFullWshBinPath)),
		"tempPath":    starbase.RemoteFullWshBinPath + ".temp",
		"installPath": starbase.RemoteFullWshBinPath,
	}
...
func NormalizeConfigPattern(pattern string) string {
	userName, err := StarSshConfigUserSettings().GetStrict(pattern, "User")
	if err != nil || userName == "" {
	...
	port, err := StarSshConfigUserSettings().GetStrict(pattern, "Port")
	if err != nil {
		port = "22"
	}
	if userName != "" {
