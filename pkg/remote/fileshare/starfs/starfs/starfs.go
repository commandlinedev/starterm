...
import (
	...
	"github.com/commandlinedev/starterm/pkg/remote/connparse"
	"github.com/commandlinedev/starterm/pkg/remote/fileshare/fspath"
	"github.com/commandlinedev/starterm/pkg/remote/fileshare/fstype"
	...
	"github.com/commandlinedev/starterm/pkg/util/tarcopy"
...
var _ fstype.FileShareClient = StarClient{}

func NewStarClient() *StarClient {
	return &StarClient{}
}

func (c StarClient) ReadStream(ctx context.Context, conn *connparse.Connection, data wshrpc.FileData) <-chan wshrpc.RespOrErrorUnion[wshrpc.FileData] {
...
func (c StarClient) Read(ctx context.Context, conn *connparse.Connection, data wshrpc.FileData) (*wshrpc.FileData, error) {
	zoneId := conn.Host
	if zoneId == "" {
...
func (c StarClient) ReadTarStream(ctx context.Context, conn *connparse.Connection, opts *wshrpc.FileCopyOpts) <-chan wshrpc.RespOrErrorUnion[iochantypes.Packet] {
...
func (c StarClient) ListEntriesStream(ctx context.Context, conn *connparse.Connection, opts *wshrpc.FileListOpts) <-chan wshrpc.RespOrErrorUnion[wshrpc.CommandRemoteListEntriesRtnData] {
...
func (c StarClient) ListEntries(ctx context.Context, conn *connparse.Connection, opts *wshrpc.FileListOpts) ([]*wshrpc.FileInfo, error) {
	...
	if err := listFilesPrefix(ctx, zoneId, prefix, func(wf *filestore.StarFile) error {
		if !opts.All {
		...
		fileList = append(fileList, starfileutil.StarFileToFileInfo(wf))
		return nil
	}); err != nil {
...
func (c StarClient) Stat(ctx context.Context, conn *connparse.Connection) (*wshrpc.FileInfo, error) {
	zoneId := conn.Host
	if zoneId == "" {
		return nil, fmt.Errorf("zoneid not found in connection")
	}
	...
	return starfileutil.StarFileToFileInfo(fileInfo), nil
}

func (c StarClient) PutFile(ctx context.Context, conn *connparse.Connection, data wshrpc.FileData) error {
	...
	wps.Broker.Publish(wps.StarEvent{
		...
		Scopes: []string{starobj.MakeORef(starobj.OType_Block, zoneId).String()},
...
func (c StarClient) AppendFile(ctx context.Context, conn *connparse.Connection, data wshrpc.FileData) error {
	dataBuf, err := base64.StdEncoding.DecodeString(data.Data64)
	if err != nil {
		return fmt.Errorf("error decoding data64: %w", err)
	}
...
// StarFile does not support directories, only prefix-based listing
func (c StarClient) Mkdir(ctx context.Context, conn *connparse.Connection) error {
	return errors.ErrUnsupported
}

func (c StarClient) MoveInternal(ctx context.Context, srcConn, destConn *connparse.Connection, opts *wshrpc.FileCopyOpts) error {
...
func (c StarClient) CopyInternal(ctx context.Context, srcConn, destConn *connparse.Connection, opts *wshrpc.FileCopyOpts) (bool, error) {
	return fsutil.PrefixCopyInternal(ctx, srcConn, destConn, c, opts, func(ctx context.Context, zoneId, prefix string) ([]string, error) {
		...
		wps.Broker.Publish(wps.StarEvent{
			Event:  wps.Event_BlockFile,
			Scopes: []string{starobj.MakeORef(starobj.OType_Block, destHost).String()},
			Data: &wps.WSFileEventData{
				ZoneId:   destHost,
...
func (c StarClient) CopyRemote(ctx context.Context, srcConn, destConn *connparse.Connection, srcClient fstype.FileShareClient, opts *wshrpc.FileCopyOpts) (bool, error) {
	if srcConn.Scheme == connparse.ConnectionTypeStar && destConn.Scheme == connparse.ConnectionTypeStar {
...
func (c StarClient) Delete(ctx context.Context, conn *connparse.Connection, recursive bool) error {
	zoneId := conn.Host
	if zoneId == "" {
		return fmt.Errorf("zoneid not found in connection")
	}
...
func listFilesPrefix(ctx context.Context, zoneId, prefix string, entryCallback func(*filestore.StarFile) error) error {
	if zoneId == "" {
		return fmt.Errorf("zoneid not found in connection")
	}
...
func (c StarClient) Join(ctx context.Context, conn *connparse.Connection, parts ...string) (*wshrpc.FileInfo, error) {
...
func (c StarClient) GetCapability() wshrpc.FileShareCapability {
...
func cleanPath(path string) (string, error) {
	...
	if strings.HasPrefix(path, "~") || strings.HasPrefix(path, ".") || strings.HasPrefix(path, "..") {
		return "", fmt.Errorf("starfile path cannot start with ~, ., or ..")
	}
	var newParts []string
...
func (c StarClient) GetConnectionType() string {
	return connparse.ConnectionTypeStar
}
