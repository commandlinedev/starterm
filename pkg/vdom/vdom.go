...
func UseState[T any](ctx context.Context, initialVal T) (T, func(T)) {
	...
	setVal := func(newVal T) {
		hookVal.Val = newVal
		vc.Root.AddRenderWork(vc.Comp.StarId)
	}
...
func UseAtom[T any](ctx context.Context, atomName string) (T, func(T)) {
	...
	if !hookVal.Init {
		hookVal.Init = true
		closedStarId := vc.Comp.StarId
		hookVal.UnmountFn = func() {
			atom := vc.Root.GetAtom(atomName)
			delete(atom.UsedBy, closedStarId)
		}
	}
	atom := vc.Root.GetAtom(atomName)
	atom.UsedBy[vc.Comp.StarId] = true
	...
	setVal := func(newVal T) {
		atom.Val = newVal
		for starId := range atom.UsedBy {
			vc.Root.AddRenderWork(starId)
		}
	}
...
func UseVDomRef(ctx context.Context) *VDomRef {
	...
	if !hookVal.Init {
		hookVal.Init = true
		refId := vc.Comp.StarId + ":" + strconv.Itoa(hookVal.Idx)
...
func UseEffect(ctx context.Context, fn func() func(), deps []any) {
	...
	if !hookVal.Init {
		...
		hookVal.Fn = fn
		hookVal.Deps = deps
		vc.Root.AddEffectWork(vc.Comp.StarId, hookVal.Idx)
		return
	}
...
func isStarTag(tag string) bool {
	return strings.HasPrefix(tag, "star:") || strings.HasPrefix(tag, "w:")
}
...
func isBaseTag(tag string) bool {
	...
	return tag[0] == '#' || unicode.IsLower(rune(tag[0])) || isStarTag(tag)
}
