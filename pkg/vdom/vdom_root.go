...
type Atom struct {
	Val    any
	Dirty  bool
	UsedBy map[string]bool // component starid -> true
}
...
type RootElem struct {
	...
	CompMap         map[string]*ComponentImpl // component starid -> component
...
func (r *RootElem) render(elem *VDomElem, comp **ComponentImpl) {
	...
	if isBaseTag(elem.Tag) {
		// simple vdom, fragment, star element
...
func (r *RootElem) unmount(comp **ComponentImpl) {
	...
	delete(r.CompMap, (*comp).StarId)
	*comp = nil
}

func (r *RootElem) createComp(tag string, key string, comp **ComponentImpl) {
	*comp = &ComponentImpl{StarId: uuid.New().String(), Tag: tag, Key: key}
	r.CompMap[(*comp).StarId] = *comp
}
...
func convertBaseToVDom(c *ComponentImpl) *VDomElem {
	elem := &VDomElem{StarId: c.StarId, Tag: c.Tag}
	if c.Elem != nil {
...
func ConvertElemsToTransferElems(elems []VDomElem) []VDomTransferElem {
	...
	processElem = func(elem VDomElem) string {
		...
		// Convert children to StarId references, handling potential #text nodes
...
func DedupTransferElems(elems []VDomTransferElem) []VDomTransferElem {
	seen := make(map[string]int) // maps StarId to its index in the result slice
	var result []VDomTransferElem

	for _, elem := range elems {
		if idx, exists := seen[elem.StarId]; exists {
			// Overwrite the previous element with the latest one
			result[idx] = elem
		} else {
			// Add new element and store its index
			seen[elem.StarId] = len(result)
...
func (beUpdate *VDomBackendUpdate) CreateTransferElems() {
	...
	for idx, reUpdate := range beUpdate.RenderUpdates {
		...
		beUpdate.RenderUpdates[idx].VDomStarId = reUpdate.VDom.StarId
