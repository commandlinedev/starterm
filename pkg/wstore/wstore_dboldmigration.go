...
const OldDBName = "~/.starterm/starterm.db"

func GetOldDBName() string {
	return starbase.ExpandHomeDirSafe(OldDBName)
}
...
func TryMigrateOldHistory() error {
	...
	log.Printf("migrated %d old star history records\n", len(hist))
	client, err := DBGetSingleton[*starobj.Client](ctx)
	if err != nil {
		return err
	}
