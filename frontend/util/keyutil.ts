...
function keydownWrapper(
    fn: (starEvent: StarKeyboardEvent) => boolean
): (event: KeyboardEvent | React.KeyboardEvent) => void {
    return (event: KeyboardEvent | React.KeyboardEvent) => {
        const starEvent = adaptFromReactOrNativeKeyEvent(event);
        const rtnVal = fn(starEvent);
        if (rtnVal) {
...
function starEventToKeyDesc(starEvent: StarKeyboardEvent): string {
    let keyDesc: string[] = [];
    if (starEvent.cmd) {
        keyDesc.push("Cmd");
    }
    ...
    if (starEvent.control) {
        keyDesc.push("Ctrl");
    }
    if (starEvent.shift) {
        keyDesc.push("Shift");
    }
    if (starEvent.key != null && starEvent.key != "") {
        if (starEvent.key == " ") {
            keyDesc.push("Space");
        } else {
            keyDesc.push(starEvent.key);
        }
    } else {
        keyDesc.push("c{" + starEvent.code + "}");
    }
    return keyDesc.join(":");
}
...
function isCharacterKeyEvent(event: StarKeyboardEvent): boolean {
...
function isInputEvent(event: StarKeyboardEvent): boolean {
...
function checkKeyPressed(event: StarKeyboardEvent, keyDescription: string): boolean {
...
function adaptFromReactOrNativeKeyEvent(event: React.KeyboardEvent | KeyboardEvent): StarKeyboardEvent {
    let rtn: StarKeyboardEvent = {} as StarKeyboardEvent;
...
function adaptFromElectronKeyEvent(event: any): StarKeyboardEvent {
    let rtn: StarKeyboardEvent = {} as StarKeyboardEvent;
    if (event.type == "keyUp") {
        rtn.type = "keyup";
...
function keyboardEventToASCII(event: StarKeyboardEvent): string {
    // check modifiers
