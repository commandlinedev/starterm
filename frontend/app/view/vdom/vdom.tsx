...
const StarTextTag = "star:text";
const StarNullTag = "star:null";
const StyleTagName = "style";
const StarStyleTagName = "star:style";

const VDomObjType_Ref = "ref";
const VDomObjType_Binding = "binding";
const VDomObjType_Func = "func";

const dlog = debug("star:vdom");

type VDomReactTagType = (props: { elem: VDomElem; model: VDomModel }) => JSX.Element;

const StarTagMap: Record<string, VDomReactTagType> = {
    "star:markdown": StarMarkdown,
};
...
function convertVDomFunc(model: VDomModel, fnDecl: VDomFunc, compId: string, propName: string): (e: any) => void {
    return (e: any) => {
        if ((propName == "onKeyDown" || propName == "onKeyDownCapture") && fnDecl["#keys"]) {
            ...
            let starEvent = adaptFromReactOrNativeKeyEvent(e);
            for (let keyDesc of fnDecl["#keys"] || []) {
                if (checkKeyPressed(starEvent, keyDesc)) {
                    e.preventDefault();
                    e.stopPropagation();
...
function convertElemToTag(elem: VDomElem, model: VDomModel): JSX.Element | string {
    ...
    return React.createElement(VDomTag, { key: elem.starid, elem, model });
}
...
function convertProps(elem: VDomElem, model: VDomModel): [GenericPropsType, Set<string>] {
    ...
    for (let key in elem.props) {
        ...
        if (isObject(val) && val.type == VDomObjType_Func) {
            ...
            props[key] = convertVDomFunc(model, valFunc, elem.starid, key);
            continue;
        }
...
function useVDom(model: VDomModel, elem: VDomElem): GenericPropsType {
    ...
    React.useEffect(() => {
        ...
        model.tagUnuseAtoms(elem.starid, oldAtomKeys);
        model.tagUseAtoms(elem.starid, atomKeys);
...
function StarMarkdown({ elem, model }: { elem: VDomElem; model: VDomModel }) {
...
function StarStyle({ src, model, onMount }: { src: string; model: VDomModel; onMount?: () => void }) {
...
function VDomTag({ elem, model }: { elem: VDomElem; model: VDomModel }) {
    const props = useVDom(model, elem);
    if (elem.tag == StarNullTag) {
        return null;
    }
    if (elem.tag == StarTextTag) {
        return props.text;
    }
    const starTag = StarTagMap[elem.tag];
    if (starTag) {
        return starTag({ elem, model });
    }
    if (elem.tag == StyleTagName) {
        return <StyleTag elem={elem} model={model} />;
    }
    if (elem.tag == StarStyleTagName) {
        return <StarStyle src={props.src} model={model} />;
    }
    ...
    return (
        <>
            {model.backendOpts?.globalstyles ? (
                <StarStyle src={model.makeVDomUrl("/star/global.css")} model={model} onMount={handleStylesMounted} />
            ) : null}
