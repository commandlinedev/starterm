...
function annotateEvent(event: VDomEvent, propName: string, reactEvent: React.SyntheticEvent) {
    ...
    if (propName == "onKeyDown") {
        const starKeyEvent = adaptFromReactOrNativeKeyEvent(reactEvent as React.KeyboardEvent);
        event.keydata = starKeyEvent;
    }
}
...
export class VDomModel {
    ...
    constructor(blockId: string, nodeModel: BlockNodeModel) {
        ...
        this.viewName = jotai.atom("Star App");
        this.backendRoute = jotai.atom((get) => {
            const blockData = get(WOS.getStarObjectAtom<Block>(makeORef("block", this.blockId)));
        ...
        this.routeGoneUnsub = starEventSubscribe({
    ...
    getBackendRoute(): string {
        const blockData = globalStore.get(WOS.getStarObjectAtom<Block>(makeORef("block", this.blockId)));
        return blockData?.meta?.["vdom:route"];
    }
    ...
    tagUseAtoms(starId: string, atomNames: Set<string>) {
        for (let atomName of atomNames) {
            let container = this.getAtomContainer(atomName);
            container.usedBy.add(starId);
        }
    }

    tagUnuseAtoms(starId: string, atomNames: Set<string>) {
        for (let atomName of atomNames) {
            let container = this.getAtomContainer(atomName);
            container.usedBy.delete(starId);
        }
    }
    ...
    handleRenderUpdates(update: VDomBackendUpdate, idMap: Map<string, VDomElem>) {
        ...
        for (let renderUpdate of update.renderupdates) {
            ...
            if (renderUpdate.updatetype == "append") {
                let parent = idMap.get(renderUpdate.starid);
                if (parent == null) {
                    this.addErrorMessage(`Could not find vdom with id ${renderUpdate.starid} (for renderupdates)`);
                    continue;
                }
    ...
    createFeUpdate(): VDomFrontendUpdate {
        ...
        const blockAtom = WOS.getStarObjectAtom<Block>(blockORef);
