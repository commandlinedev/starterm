...
export function restoreVDomElems(backendUpdate: VDomBackendUpdate) {
    ...
    // Step 1: Map of starid to VDomElem, skipping any without a starid
    const elemMap = new Map<string, VDomElem>();
    backendUpdate.transferelems.forEach((transferElem) => {
        if (!transferElem.starid) {
            return;
        }
        elemMap.set(transferElem.starid, {
            starid: transferElem.starid,
    ...
    backendUpdate.transferelems.forEach((transferElem) => {
        const parent = elemMap.get(transferElem.starid);
    ...
    backendUpdate.renderupdates.forEach((update) => {
        if (update.vdomstarid) {
            update.vdom = elemMap.get(update.vdomstarid);
        }
    });
}
