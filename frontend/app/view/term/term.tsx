...
import { computeBgStyleFromMeta } from "@/util/starutil";
...
class TermViewModel implements ViewModel {
    ...
    constructor(blockId: string, nodeModel: BlockNodeModel) {
        ...
        this.shellProcStatusUnsubFn = starEventSubscribe({
    ...
    keyDownHandler(starEvent: StarKeyboardEvent): boolean {
        if (keyutil.checkKeyPressed(starEvent, "Cmd:Escape")) {
            const blockAtom = WOS.getStarObjectAtom<Block>(`block:${this.blockId}`);
        ...
        if (blockData.meta?.["term:mode"] == "vdom") {
            ...
            return vdomModel?.keyDownHandler(starEvent);
        }
        return false;
    }

    handleTerminalKeydown(event: KeyboardEvent): boolean {
        const starEvent = keyutil.adaptFromReactOrNativeKeyEvent(event);
        if (starEvent.type != "keydown") {
            return true;
        }
        if (this.keyDownHandler(starEvent)) {
            event.preventDefault();
            event.stopPropagation();
            return false;
        }
        // deal with terminal specific keybindings
        if (keyutil.checkKeyPressed(starEvent, "Ctrl:Shift:v")) {
            ...
            event.stopPropagation();
            return false;
        } else if (keyutil.checkKeyPressed(starEvent, "Ctrl:Shift:c")) {
            const sel = this.termRef.current?.terminal.getSelection();
            navigator.clipboard.writeText(sel);
            event.preventDefault();
            event.stopPropagation();
            return false;
        } else if (keyutil.checkKeyPressed(starEvent, "Cmd:k")) {
            event.preventDefault();
            event.stopPropagation();
        ...
        if ((shellProcStatus == "done" || shellProcStatus == "init") && keyutil.checkKeyPressed(starEvent, "Enter")) {
            this.forceRestartController();
            return false;
        }
        const appHandled = appHandleKeyDown(starEvent);
        if (appHandled) {
            event.preventDefault();
            event.stopPropagation();
            return false;
...
const TermVDomToolbarNode = ({ vdomBlockId, blockId, model }: TerminalViewProps & { vdomBlockId: string }) => {
    React.useEffect(() => {
        const unsub = starEventSubscribe({
...
const TerminalView = ({ blockId, model }: ViewComponentProps<TermViewModel>) => {
    ...
    const [blockData] = WOS.useStarObjectValue<Block>(WOS.makeORef("block", blockId));
