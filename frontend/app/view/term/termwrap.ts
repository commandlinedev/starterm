...
import { WOS, atoms, fetchStarFile, getSettingsKeyAtom, globalStore, openLink } from "@/store/global";
...
const dlog = debug("star:termwrap");
...
function handleOscStarCommand(data: string, blockId: string, loaded: boolean): boolean {
    if (!loaded) {
        return false;
    }
    if (!data || data.length === 0) {
        console.log("Invalid Star OSC command received (empty)");
        return false;
    }

    // Expected formats:
    // "setmeta;{JSONDATA}"
    // "setmeta;[star-id];{JSONDATA}"
    const parts = data.split(";");
    if (parts[0] !== "setmeta") {
        console.log("Invalid Star OSC command received (bad command)", data);
        return false;
    }
    ...
    if (parts.length === 2) {
        ...
        console.log("Invalid Star OSC command received (1 part)", data);
        return false;
    }
    ...
    try {
        ...
        console.error("Invalid JSON in Star OSC command:", e);
        return false;
    }

    if (starId) {
        // Resolve the star id to an ORef using our ResolveIdsCommand.
        fireAndForget(() => {
            return RpcApi.ResolveIdsCommand(TabRpcClient, { blockid: blockId, ids: [starId] })
                .then((response: { resolvedids: { [key: string]: any } }) => {
                    const oref = response.resolvedids[starId];
                    if (!oref) {
                        console.error("Failed to resolve star id:", starId);
                        return;
                    }
                    services.ObjectService.UpdateObjectMeta(oref, meta);
                })
                .catch((err: any) => {
                    console.error("Error resolving star id", starId, err);
                });
        });
    } else {
        // No star id provided; update using the current block id.
        fireAndForget(() => {
...
export class TermWrap {
    ...
    constructor(
        ...
        starOptions: TermWrapOptions
    ) {
        this.loaded = false;
        this.blockId = blockId;
        this.sendDataHandler = starOptions.sendDataHandler;
        this.ptyOffset = 0;
        ...
        if (WebGLSupported && starOptions.useWebGl) {
        ...
        this.terminal.parser.registerOscHandler(9283, (data: string) => {
            return handleOscStarCommand(data, this.blockId, this.loaded);
        });
        this.terminal.parser.registerOscHandler(7, (data: string) => {
            return handleOsc7Command(data, this.blockId, this.loaded);
        });
        this.terminal.attachCustomKeyEventHandler(starOptions.keydownHandler);
    ...
    async loadInitialTerminalData(): Promise<void> {
        ...
        const { data: cacheData, fileInfo: cacheFile } = await fetchStarFile(this.blockId, TermCacheFileName);
        let ptyOffset = 0;
        if (cacheFile != null) {
        ...
        const { data: mainData, fileInfo: mainFile } = await fetchStarFile(this.blockId, TermFileName, ptyOffset);
        console.log(
