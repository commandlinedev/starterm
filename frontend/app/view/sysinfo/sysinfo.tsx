...
function convertStarEventToDataItem(event: StarEvent): DataItem {
...
class SysinfoViewModel implements ViewModel {
    ...
    async loadInitialData() {
        ...
        try {
            ...
            const initialDataItems: DataItem[] = initialData.map(convertStarEventToDataItem);
...
function SysinfoView({ model, blockId }: SysinfoViewProps) {
    ...
    React.useEffect(() => {
        const unsubFn = starEventSubscribe({
            eventType: "sysinfo",
            scope: connName,
            handler: (event) => {
                ...
                const dataItem = convertStarEventToDataItem(event);
