...
import { atoms, createBlock, fetchStarFile, getApi, globalStore, WOS } from "@/store/global";
...
function promptToMsg(prompt: StarAIPromptMessageType): ChatMessageType {
    return {
...
class AiWshClient extends WshClient {
    ...
    constructor(blockId: string, model: StarAiModel) {
...
export class StarAiModel implements ViewModel {
    viewType: string;
    blockId: string;
    ...
    constructor(blockId: string) {
        ...
        this.cancel = false;
        this.viewType = "starai";
        this.blockId = blockId;
        this.blockAtom = WOS.getStarObjectAtom<Block>(`block:${blockId}`);
        this.viewIcon = atom("sparkles");
        this.viewName = atom("Star AI");
                ...
                default:
                    if (isCloud) {
                        viewTextChildren.push({
                            elemtype: "iconbutton",
                            icon: "cloud",
                            title: "Using Star's AI Proxy (gpt-4o-mini)",
                            noAction: true,
                        });
                    } else {
    ...
    async fetchAiData(): Promise<Array<StarAIPromptMessageType>> {
        const { data } = await fetchStarFile(this.blockId, "aidata");
        if (!data) {
            return [];
        }
        const history: Array<StarAIPromptMessageType> = JSON.parse(new TextDecoder().decode(data));
    ...
    sendMessage(text: string, user: string = "user") {
        ...
        const newPrompt: StarAIPromptMessageType = {
            role: "user",
            content: text,
        };
        const handleAiStreamingResponse = async () => {
            ...
            const history = await this.fetchAiData();
            const beMsg: StarAIStreamRequest = {
                clientid: clientId,
                opts: opts,
            ...
            let fullMsg = "";
            try {
                const aiGen = RpcApi.StreamStarAiCommand(TabRpcClient, beMsg, { timeout: opts.timeoutms });
                ...
                if (fullMsg == "") {
                    ...
                    await BlockService.SaveStarAiData(this.blockId, [...history, newPrompt]);
                } else {
                    const responsePrompt: StarAIPromptMessageType = {
                        role: "assistant",
                        content: fullMsg,
                    };
                    ...
                    await BlockService.SaveStarAiData(this.blockId, [...history, newPrompt, responsePrompt]);
                }
            } catch (error) {
                ...
                globalStore.set(this.updateLastMessageAtom, "", false);
                const errorPrompt: StarAIPromptMessageType = {
                    role: "error",
                    content: errMsg,
                };
                updatedHist.push(errorPrompt);
                await BlockService.SaveStarAiData(this.blockId, updatedHist);
            }
    ...
    async clearMessages() {
        await BlockService.SaveStarAiData(this.blockId, []);
        globalStore.set(this.messagesAtom, []);
    }

    keyDownHandler(starEvent: StarKeyboardEvent): boolean {
        if (checkKeyPressed(starEvent, "Cmd:l")) {
...
const StarAi = ({ model }: { model: StarAiModel; blockId: string }) => {
    const { sendMessage } = model.useStarAi();
    const staraiRef = useRef<HTMLDivElement>(null);
    ...
    const handleTextAreaKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        const starEvent = adaptFromReactOrNativeKeyEvent(e);
        if (checkKeyPressed(starEvent, "Enter")) {
            e.preventDefault();
            handleEnterKeyPressed();
        } else if (checkKeyPressed(starEvent, "ArrowUp")) {
            handleArrowUpPressed(e);
        } else if (checkKeyPressed(starEvent, "ArrowDown")) {
            handleArrowDownPressed(e);
        }
    };

    let buttonClass = "starai-submit-button";
    let buttonIcon = makeIconClass("arrow-up", false);
    let buttonTitle = "run";
    if (locked) {
        buttonClass = "starai-submit-button stop";
    ...
    return (
        <div ref={staraiRef} className="starai">
            <div className="starai-chat">
                <ChatWindow ref={osRef} chatWindowRef={chatWindowRef} msgWidths={msgWidths} model={model} />
            </div>
            <div className="starai-controls">
                <div className="starai-input-wrapper">
                    <ChatInput
                        ref={inputRef}
                        value={value}
                        model={model}
