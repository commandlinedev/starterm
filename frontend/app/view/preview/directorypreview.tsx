...
import { formatRemoteUri } from "@/util/starutil";
...
function DirectoryPreview({ model }: DirectoryPreviewProps) {
    ...
    useEffect(() => {
        model.directoryKeyDownHandler = (starEvent: StarKeyboardEvent): boolean => {
            if (checkKeyPressed(starEvent, "Escape")) {
                setSearchText("");
                return;
            }
            if (checkKeyPressed(starEvent, "ArrowUp")) {
                setFocusIndex((idx) => Math.max(idx - 1, 0));
                return true;
            }
            if (checkKeyPressed(starEvent, "ArrowDown")) {
                setFocusIndex((idx) => Math.min(idx + 1, filteredData.length - 1));
                return true;
            }
            if (checkKeyPressed(starEvent, "PageUp")) {
                setFocusIndex((idx) => Math.max(idx - PageJumpSize, 0));
                return true;
            }
            if (checkKeyPressed(starEvent, "PageDown")) {
            ...
            if (checkKeyPressed(starEvent, "Backspace")) {
            ...
            if (
                checkKeyPressed(starEvent, "Space") &&
                searchText == "" &&
            ...
            if (isCharacterKeyEvent(starEvent)) {
                setSearchText((current) => current + starEvent.key);
                return true;
            }
            return false;
        };
