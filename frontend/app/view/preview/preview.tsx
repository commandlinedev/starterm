...
export class PreviewModel implements ViewModel {
    ...
    directoryKeyDownHandler: (starEvent: StarKeyboardEvent) => boolean;
    codeEditKeyDownHandler: (starEvent: StarKeyboardEvent) => boolean;
...
function StreamingPreview({ model }: SpecializedViewProps) {
    ...
    const streamingUrl = `${getWebServerEndpoint()}/star/stream-file?${usp.toString()}`;
...
function CodeEditPreview({ model }: SpecializedViewProps) {
    ...
    function codeEditKeyDownHandler(e: StarKeyboardEvent): boolean {
        if (checkKeyPressed(e, "Cmd:e")) {
    ...
    function onMount(editor: MonacoTypes.editor.IStandaloneCodeEditor, monaco: Monaco): () => void {
        ...
        editor.onKeyDown((e: MonacoTypes.IKeyboardEvent) => {
            const starEvent = adaptFromReactOrNativeKeyEvent(e.browserEvent);
            const handled = tryReinjectKey(starEvent);
            if (handled) {
                e.stopPropagation();
                e.preventDefault();
            }
