...
import { getStarObjectAtom, makeORef, useStarObjectValue } from "@/store/wos";
...
import { StarAiModel } from "@/view/starai/starai";
...
BlockRegistry.set("starai", StarAiModel);
...
function makeDefaultViewModel(blockId: string, viewType: string): ViewModel {
    const blockDataAtom = getStarObjectAtom<Block>(makeORef("block", blockId));
...
const BlockPreview = memo(({ nodeModel, viewModel }: FullBlockProps) => {
    const [blockData] = useStarObjectValue<Block>(makeORef("block", nodeModel.blockId));
    if (!blockData) {
        return null;
    }
    return (
...
const Block = memo((props: BlockProps) => {
    ...
    const [blockData, loading] = useStarObjectValue<Block>(makeORef("block", props.nodeModel.blockId));
