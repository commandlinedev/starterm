...
const ChangeConnectionBlockModal = React.memo(
    ({
        ...
        const handleTypeAheadKeyDown = React.useCallback(
            (starEvent: StarKeyboardEvent): boolean => {
                if (keyutil.checkKeyPressed(starEvent, "Enter")) {
                ...
                if (keyutil.checkKeyPressed(starEvent, "Escape")) {
                    globalStore.set(changeConnModalAtom, false);
                    setConnSelected("");
                    globalRefocusWithTimeout(10);
                    return true;
                }
                if (keyutil.checkKeyPressed(starEvent, "ArrowUp")) {
                    setRowIndex((idx) => Math.max(idx - 1, 0));
                    return true;
                }
                if (keyutil.checkKeyPressed(starEvent, "ArrowDown")) {
