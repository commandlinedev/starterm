...
class ObjectServiceType {
    ...
    // get star object by oref
    GetObject(oref: string): Promise<StarObj> {
        return WOS.callBackendService("object", "GetObject", Array.from(arguments))
    }

    // @returns objects
    GetObjects(orefs: string[]): Promise<StarObj[]> {
        return WOS.callBackendService("object", "GetObjects", Array.from(arguments))
    }

    // @returns object updates
    UpdateObject(starObj: StarObj, returnUpdates: boolean): Promise<void> {
...
class WindowServiceType {
    ...
    CreateWindow(winSize: WinSize, workspaceId: string): Promise<StarWindow> {
        return WOS.callBackendService("window", "CreateWindow", Array.from(arguments))
    }
    GetWindow(windowId: string): Promise<StarWindow> {
