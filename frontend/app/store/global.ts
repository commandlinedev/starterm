...
import { getFileSubject, starEventSubscribe } from "./wps";
...
function initGlobalAtoms(initOpts: GlobalInitOptions) {
    ...
    const windowDataAtom: Atom<StarWindow> = atom((get) => {
        const windowId = get(windowIdAtom);
        if (windowId == null) {
            return null;
        }
        const rtn = WOS.getObjectValue<StarWindow>(WOS.makeORef("window", windowId), get);
        return rtn;
    });
    ...
    atoms = {
        // initialized in star.ts (will not be null inside of application)
        clientId: clientIdAtom,
        uiContext: uiContextAtom,
        client: clientAtom,
        starWindow: windowDataAtom,
...
function initGlobalStarEventSubs(initOpts: StarInitOpts) {
    starEventSubscribe(
        {
            eventType: "starobj:update",
            handler: (event) => {
                // console.log("starobj:update star event handler", event);
                const update: StarObjUpdate = event.data;
                WOS.updateStarObject(update);
            },
        },
        {
            eventType: "config",
            handler: (event) => {
                // console.log("config star event handler", event);
...
function getBlockMetaKeyAtom<T extends keyof MetaType>(blockId: string, key: T): Atom<MetaType[T]> {
    ...
    metaAtom = atom((get) => {
        let blockAtom = WOS.getStarObjectAtom(WOS.makeORef("block", blockId));
...
function useBlockDataLoaded(blockId: string): boolean {
    const loadedAtom = useBlockAtom<boolean>(blockId, "block-loaded", () => {
        return WOS.getStarObjectLoadingAtom(WOS.makeORef("block", blockId));
    });
...
async function fetchStarFile(
    zoneId: string,
    fileName: string,
    offset?: number
): Promise<{ data: Uint8Array; fileInfo: StarFile }> {
    ...
    const resp = await fetch(getWebServerEndpoint() + "/star/file?" + usp.toString());
    if (!resp.ok) {
        if (resp.status === 404) {
            return { data: null, fileInfo: null };
        }
        throw new Error("error getting star file: " + resp.statusText);
    }
    if (resp.status == 204) {
        return { data: null, fileInfo: null };
    }
