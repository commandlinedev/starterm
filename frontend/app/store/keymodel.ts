...
type KeyHandler = (event: StarKeyboardEvent) => boolean;

const simpleControlShiftAtom = jotai.atom(false);
const globalKeyMap = new Map<string, (starEvent: StarKeyboardEvent) => boolean>();
...
function shouldDispatchToBlock(e: StarKeyboardEvent): boolean {
...
function genericClose(tabId: string) {
    ...
    const tabAtom = WOS.getStarObjectAtom<Tab>(tabORef);
...
function getDefaultNewBlockDef(): BlockDef {
    ...
    const focusedNode = globalStore.get(layoutModel.focusedNode);
    if (focusedNode != null) {
        const blockAtom = WOS.getStarObjectAtom<Block>(WOS.makeORef("block", focusedNode.data?.blockId));
...
// returns [keymatch, T]
function checkKeyMap<T>(starEvent: StarKeyboardEvent, keyMap: Map<string, T>): [string, T] {
    for (const key of keyMap.keys()) {
        if (keyutil.checkKeyPressed(starEvent, key)) {
...
function appHandleKeyDown(starEvent: StarKeyboardEvent): boolean {
    const nativeEvent = (starEvent as any).nativeEvent;
    ...
    if (activeChord) {
        ...
        const [, handler] = checkKeyMap(starEvent, chordBindings);
        if (handler) {
            resetChord();
            return handler(starEvent);
        } else {
    ...
    const [chordKeyMatch] = checkKeyMap(starEvent, globalChordMap);
    if (chordKeyMatch) {
        setActiveChord(chordKeyMatch);
        return true;
    }

    const [, globalHandler] = checkKeyMap(starEvent, globalKeyMap);
    if (globalHandler) {
        const handled = globalHandler(starEvent);
        if (handled) {
            return true;
        }
    }
    ...
    const focusedNode = globalStore.get(layoutModel.focusedNode);
    const blockId = focusedNode?.data?.blockId;
    if (blockId != null && shouldDispatchToBlock(starEvent)) {
        const bcm = getBlockComponentModel(blockId);
        const viewModel = bcm?.viewModel;
        if (viewModel?.keyDownHandler) {
            const handledByBlock = viewModel.keyDownHandler(starEvent);
            if (handledByBlock) {
                return true;
            }
        }
...
function registerElectronReinjectKeyHandler() {
    getApi().onReinjectKey((event: StarKeyboardEvent) => {
        appHandleKeyDown(event);
    });
}

function tryReinjectKey(event: StarKeyboardEvent): boolean {
...
function registerGlobalKeys() {
    ...
    function activateSearch(event: StarKeyboardEvent): boolean {
        const bcm = getBlockComponentModel(getFocusedBlockInStaticTab());
...
function handleGlobalStarKeyboardEvents(starEvent: StarKeyboardEvent): boolean {
