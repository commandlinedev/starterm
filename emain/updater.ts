...
import { focusedStarWindow, getAllStarWindows } from "./emain-window";
...
export class Updater {
    ...
    constructor(settings: SettingsType) {
        ...
        autoUpdater.on("update-downloaded", (event) => {
            ...
            const updateNotification = new Notification({
                title: "Star Terminal",
                body: "A new version of Star Terminal is ready to install.",
            });
    ...
    private set status(value: UpdaterStatus) {
        this._status = value;
        getAllStarWindows().forEach((window) => {
        ...
        if (
            ...
            if (userInput && !result.downloadPromise) {
                ...
                if (focusedStarWindow) {
                    dialog.showMessageBox(focusedStarWindow, dialogOpts);
                }
            }
    ...
    async promptToInstallUpdate() {
        ...
        if (allWindows.length > 0) {
            await dialog.showMessageBox(focusedStarWindow ?? allWindows[0], dialogOpts).then(({ response }) => {
                if (response === 0) {
