...
import { createBrowserWindow, getStarWindowById, getStarWindowByWorkspaceId } from "./emain-window";
...
export class ElectronWshClientType extends WshClient {
    ...
    async handle_webselector(rh: RpcResponseHelper, data: CommandWebSelectorData): Promise<string[]> {
        ...
        const ww = getStarWindowByWorkspaceId(data.workspaceid);
        if (ww == null) {
    ...
    async handle_notify(rh: RpcResponseHelper, notificationOptions: StarNotificationOptions) {
        new Notification({
    ...
    async handle_focuswindow(rh: RpcResponseHelper, windowId: string) {
        ...
        let ww = getStarWindowById(windowId);
        if (ww == null) {
