...
export const StarAppPathVarName = "STARTERM_APP_PATH";
...
export function handleCtrlShiftState(sender: Electron.WebContents, starEvent: StarKeyboardEvent) {
    if (starEvent.type == "keyup") {
        if (starEvent.key === "Control" || starEvent.key === "Shift") {
            setCtrlShift(sender, false);
        }
        if (starEvent.key == "Meta") {
            if (starEvent.control && starEvent.shift) {
    ...
    if (starEvent.type == "keydown") {
        if (starEvent.key === "Control" || starEvent.key === "Shift" || starEvent.key === "Meta") {
            if (starEvent.control && starEvent.shift && !starEvent.meta) {
...
export function shFrameNavHandler(event: Electron.Event<Electron.WebContentsWillFrameNavigateEventParams>) {
    ...
    if (
        ...
        (url.startsWith("blob:file:///") ||
            url.startsWith(getWebServerEndpoint() + "/star/stream-file?") ||
            url.startsWith(getWebServerEndpoint() + "/star/stream-file/") ||
            url.startsWith(getWebServerEndpoint() + "/star/stream-local-file?"))
    ) {
        // allowed
        return;
    }
...
export function starKeyToElectronKey(starKey: string): string {
    const starParts = starKey.split(":");
    const electronParts: Array<string> = starParts.map((part: string) => {
